...
export const DEFAULT_MODEL_BY_PROVIDER: Partial<Record<ProviderDriverKind, string>> = {
  ...
  // Hermes model ids are `<provider>:<model>` (server.py `_encode_model_choice`).
  // Its own silent default is GLM on OpenRouter; the live catalog from
  // `session/new` replaces this the moment a session starts.
  [HERMES_DRIVER_KIND]: "openrouter:z-ai/glm-5.2",
