/**
 * HermesTextGeneration — commit messages, PR content, branch names, and thread
 * titles generated through a throwaway Hermes ACP session.
 *
 * Same shape as the other ACP-backed generators: one short-lived session per
 * call, output collected from `agent_message_chunk` notifications, JSON
 * decoded against the caller's schema.
 *
 * Two Hermes-specific details:
 *
 * - **Output is read from a raw `session/update` handler, not the parsed event
 *   stream.** The stream is turn-scoped and needs an active turn; here there
 *   is only a single prompt whose whole answer is the payload.
 *
 * - **The timeout is generous.** Hermes runs prompts on a four-worker pool
 *   shared across every session in the process, so a text-generation call can
 *   queue behind real work. Three minutes matches the other ACP generators and
 *   leaves room for that queue without hanging a commit dialog forever.
 *
 * Note that the session Hermes creates here is persisted to its `state.db` like
 * any other, tagged `source="acp"`. That is Hermes' design — every ACP session
 * is durable — and it is why these sessions use their own client name, so they
 * are identifiable in `hermes` session listings.
 *
        ...
        return yield* new TextGenerationError({
          ...
          detail:
            promptResult.stopReason === "cancelled"
              ...
              : // `refusal` is what Hermes returns for an unknown session id,
                // so it is worth naming separately from a genuinely empty
                // answer.
                promptResult.stopReason === "refusal"
                ? "Hermes refused the text generation request."
