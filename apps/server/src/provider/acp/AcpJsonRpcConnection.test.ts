...
describe("AcpSessionRuntime", () => {
  ...
  it.effect("keeps thought chunks on their own parsed event", () =>
    Effect.gen(function* () {
      const runtime = yield* AcpSessionRuntime.AcpSessionRuntime;
      yield* runtime.start();
      ...
      const notes = Array.from(yield* Stream.runCollect(Stream.take(runtime.getEvents(), 4)));
      const deltas = notes.filter(
        (note) => note._tag === "ThoughtDelta" || note._tag === "ContentDelta",
      );
      expect(deltas.map((note) => [note._tag, note.text])).toEqual([
