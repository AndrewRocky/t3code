/**
 * HermesDriver — instance factory for the Hermes Agent provider.
 *
 * Follows the standard driver bundle: one adapter, one text-generation
 * service, and one managed snapshot per instance, all owned by the scope the
 * registry passes in.
 *
 * Maintenance is manual-only and carries no package name. Hermes is not
 * installed from npm — its installer script manages a `uv`-owned Python
 * environment under the Hermes home and `hermes update` is the upgrade path —
 * so there is no registry T3 Code could poll for a newer version. The version
 * advisory still runs, driven entirely by what `hermes --version` reports.
 *
...
// Hermes installs from a Python checkout, so no package manager owns the CLI
// and there is nothing to resolve per instance. Same shape as Grok.
...
export const HermesDriver: ProviderDriver<HermesSettings, HermesDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const checkProvider = checkHermesProviderStatus(effectiveConfig, processEnv, cwd).pipe(
